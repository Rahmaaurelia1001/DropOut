...
<div class="da-shell">
    ...
    <aside class="da-sidebar">
        ...
        <div class="sb-nav">
            ...
            <a href="{{ route('admin.mapel.index') }}" class="sb-item active {{ request()->routeIs('admin.mapel.*') ? 'active' : '' }}">
    ...
    <main class="da-main">
        <div class="da-phead">
            <div>
                <h2 class="da-phead-title">Manajemen Mata Pelajaran</h2>
                <p style="font-size:12px; color:var(--gray-400); margin-top:2px">Daftar mata pelajaran yang digunakan dalam proses penilaian</p>
            </div>
            <a href="{{ route('admin.mapel.create') }}" class="btn-add">
        ...
        <div class="da-content">
            ...
            <div class="table-card">
                <div class="table-card-head">
                    <span style="font-weight: 800; font-size: 13px;">Data Mata Pelajaran</span>
                    <span style="font-size: 11px; color: var(--gray-400); background: var(--gray-100); padding: 2px 9px; border-radius: 99px;">{{ $mapel->count() }} mapel</span>
                </div>
                    ...
                    <table class="spk-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="font-weight:700; color:var(--gray-900)">{{ $item->nama_mapel }}</td>
                                <td>
                                    <div style="display:flex; justify-content:center; gap:8px">
                                        <a href="{{ route('admin.mapel.edit', $item->id_mapel) }}" class="act-btn act-edit">Edit</a>
                                        <form id="delete-mapel-{{ $item->id_mapel }}" action="{{ route('admin.mapel.destroy', $item->id_mapel) }}" method="POST" style="margin:0">
                                            @csrf @method('DELETE')
                                        </form>
                                        <button class="act-btn act-delete" onclick="confirmDelete('delete-mapel-{{ $item->id_mapel }}', '{{ $item->nama_mapel }}')">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
