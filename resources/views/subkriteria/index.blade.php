...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-add { display: inline-flex; align-items: center; gap: 7px; padding: 9px 16px; border-radius: 9px; background: var(--blue); color: var(--white); font-size: 13px; font-weight: 700; text-decoration: none; transition: 0.15s; border: none; cursor: pointer; }
    ...
    .spk-table th { padding: 12px 20px; font-size: 10.5px; font-weight: 700; color: var(--gray-400); text-transform: uppercase; text-align: left; letter-spacing: 0.07em; }
    .spk-table td { padding: 14px 20px; font-size: 13px; border-bottom: 1px solid var(--gray-100); color: var(--gray-700); }
    
    /* Dynamic Badges */
    .badge-base { font-size: 10px; font-weight: 800; padding: 4px 10px; border-radius: 6px; text-transform: uppercase; border: 1px solid transparent; }
    
    .badge-0 { color: #2563eb; background: #eff6ff; border-color: #dbeafe; } /* Biru */
    .badge-1 { color: #059669; background: #ecfdf5; border-color: #d1fae5; } /* Hijau */
    .badge-2 { color: #7c3aed; background: #f5f3ff; border-color: #ede9fe; } /* Ungu */
    .badge-3 { color: #db2777; background: #fdf2f8; border-color: #fce7f3; } /* Pink */
    .badge-4 { color: #ea580c; background: #fff7ed; border-color: #ffedd5; } /* Oranye */
    .badge-5 { color: #0891b2; background: #ecfeff; border-color: #cffafe; } /* Cyan */
...
@php
    // Helper untuk merotasi warna berdasarkan ID kriteria
    $colors = ['badge-0', 'badge-1', 'badge-2', 'badge-3', 'badge-4', 'badge-5'];
@endphp
...
<div class="da-shell">
    ...
    <aside class="da-sidebar">
        ...
        <div class="sb-nav">
            ...
            <a href="{{ route('admin.subkriteria.index') }}" class="sb-item active"><svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M4 6h16M4 10h16M4 14h10M4 18h6"/></svg>Data Subkriteria</a>
    ...
    <main class="da-main">
        <div class="da-phead">
            <div>
                <h2 class="da-phead-title">Data Subkriteria</h2>
                <p style="font-size:12px; color:var(--gray-400); margin-top:2px">Kelola nilai skala tiap kriteria penilaian secara detail</p>
            </div>
            <a href="{{ route('admin.subkriteria.create') }}" class="btn-add">
        ...
        <div class="da-content">
            ...
            <div class="table-card">
                <div class="table-card-head">
                    <span style="font-weight: 800; font-size: 13px;">Daftar Detail Subkriteria</span>
                    <span style="font-size: 11px; color: var(--gray-400); background: var(--gray-100); padding: 2px 9px; border-radius: 99px;">{{ $subkriteria->count() }} item</span>
                </div>
                    ...
                    <table class="spk-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    {{-- Pewarnaan badge berdasarkan ID Kriteria --}}
                                    @php $colorClass = $colors[$s->id_kriteria % count($colors)]; @endphp
                                    <span class="badge-base {{ $colorClass }}">
                                        {{ $s->kriteria->nama_kriteria }}
                                    </span>
                                </td>
                                <td style="font-weight:700; color:var(--gray-900)">{{ $s->nama_subkriteria }}</td>
                                <td>
                                    <span style="font-weight: 800; color: var(--blue); background: var(--blue-lt); padding: 4px 12px; border-radius: 6px;">
                                        {{ $s->nilai_skala }}
                                    </span>
                                </td>
                                <td>
