...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .form-control { width: 100%; padding: 10px 12px; border: 1.5px solid var(--gray-200); border-radius: 9px; font-size: 13px; font-weight: 500; font-family: inherit; outline: none; transition: all .15s; background: white; }
    ...
    .preview-kelas { background: var(--blue-lt); border: 1.5px solid var(--blue-mid); border-radius: 10px; padding: 12px 16px; display: flex; align-items: center; gap: 12px; }
    .preview-badge { font-size: 22px; font-weight: 800; color: var(--blue); }
    .preview-label { font-size: 11px; color: #3b82f6; font-weight: 600; }
...
<div class="da-shell">
    ...
    <aside class="da-sidebar">
        ...
        <div class="sb-nav">
            ...
            <a href="{{ route('admin.kelas.index') }}" class="sb-item active"><svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>Data Kelas</a>
    ...
    <main class="da-main">
        <div class="da-phead">
            <a href="{{ route('admin.kelas.index') }}" class="da-phead-back">
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M15 19l-7-7 7-7"/></svg>
            </a>
            <div>
                <h2 class="da-phead-title">Edit Data Kelas</h2>
                <p style="font-size:12px; color:var(--gray-400); margin-top:2px;">Perbarui tingkatan, huruf, atau tahun ajaran kelas</p>
            </div>
        </div>

        <div class="da-content">
            @php
                // Pisah nama_kelas menjadi level dan huruf
                // nama_kelas format: "1A", "2B", "3C" dst
                $currentLevel = preg_replace('/[^0-9]/', '', $kelas->nama_kelas);
                $currentHuruf = strtoupper(preg_replace('/[^a-zA-Z]/', '', $kelas->nama_kelas));
            @endphp
            ...
            <form action="{{ route('admin.kelas.update', $kelas->id_kelas) }}" method="POST">
                ...
                <div class="form-card">
                    <div class="form-card-head">
                        ...
                        <div>
                            <div style="font-weight:800; font-size:14px;">Informasi Kelas</div>
                            <div style="font-size:11px; color:var(--gray-400);">Perbarui tingkatan dan huruf kelas</div>
                        </div>
                    </div>
                    ...
                    <div class="form-body">
                        ...
                        {{-- Preview --}}
                        <div class="preview-kelas">
                            <div class="preview-badge" id="previewNama">{{ $kelas->nama_kelas }}</div>
                            <div>
                                <div class="preview-label">Nama kelas yang akan disimpan</div>
                                <div style="font-size:12px; color:#1d4ed8; font-weight:500;">Sebelumnya: <strong>{{ $kelas->nama_kelas }}</strong></div>
                            </div>
                        </div>
                        ...
                        <div class="form-row-3">
                            ...
                            <div class="form-group">
                                ...
                                <select name="level_kelas" id="level_kelas" class="form-control @error('level_kelas') is-error @enderror" onchange="updatePreview()">
                                    <option value="">-- Pilih --</option>
                                    @for($i = 1; $i <= 6; $i++)
                                        <option value="{{ $i }}" {{ (old('level_kelas', $currentLevel) == $i) ? 'selected' : '' }}>Kelas {{ $i }}</option>
                                    @endfor
                                </select>
                                @error('level_kelas')
                                    <span class="form-error"><svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/></svg>{{ $message }}</span>
                                @enderror
                            </div>

                            {{-- Huruf --}}
                            <div class="form-group">
                                <label class="form-label">Huruf Kelas</label>
                                <select name="huruf_kelas" id="huruf_kelas" class="form-control @error('huruf_kelas') is-error @enderror" onchange="updatePreview()">
                                    <option value="">-- Pilih --</option>
                                    <option value="A" {{ old('huruf_kelas', $currentHuruf) === 'A' ? 'selected' : '' }}>A</option>
                                    <option value="B" {{ old('huruf_kelas', $currentHuruf) === 'B' ? 'selected' : '' }}>B</option>
                                    <option value="C" {{ old('huruf_kelas', $currentHuruf) === 'C' ? 'selected' : '' }}>C</option>
                                    <option value="D" {{ old('huruf_kelas', $currentHuruf) === 'D' ? 'selected' : '' }}>D</option>
                                </select>
                                @error('huruf_kelas')
                                    <span class="form-error"><svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/></svg>{{ $message }}</span>
                                @enderror
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="tahun_ajaran" class="form-control @error('tahun_ajaran') is-error @enderror"
...
<script>
    function updatePreview() {
        var level = document.getElementById('level_kelas').value;
        var huruf = document.getElementById('huruf_kelas').value;
        var preview = document.getElementById('previewNama');
        if (level && huruf) {
            preview.textContent = level + huruf.toUpperCase();
        } else if (level) {
            preview.textContent = level + '?';
        } else {
            preview.textContent = '—';
        }
    }
