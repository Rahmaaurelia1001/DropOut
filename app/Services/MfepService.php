...
class MfepService
{
        ...
        foreach ($penilaians as $idSiswa => $items) {
            ...
            foreach ($items as $item) {
                ...
                if ($hasilPerkalian > $nilaiDominan) {
                    ...
                    $idPerhitunganDominan = $perhitungan->id_perhitungan;
                }
            }
            ...
            $hasilSementara[] = [
                ...
                'id_perhitungan'         => $idPerhitunganDominan,
            ];
        }
        ...
        foreach ($hasilSementara as $hasil) {
            ...
            $hasilModel = HasilKeputusan::updateOrCreate(
                ...
                [
                    ...
                    'tanggal_proses'         => now(),
                    'id_perhitungan'         => $hasil['id_perhitungan'],
                ]
            );
    ...
    {
        if ($totalKetidakhadiran === null) return 0;
    ...
    private function mapPekerjaanOrtu(?string $pekerjaan): int
    {
        ...
        if (in_array($pekerjaan, ['ASN', 'PNS'])) return 5;

        if (in_array($pekerjaan, [
            'PENJAGA SEKOLAH', 'SATPAM', 'KARYAWAN TIDAK TETAP',
            'PEKERJAAN FORMAL', 'FORMAL', 'ASN/PEKERJAAN FORMAL', 'ASN/FORMAL'
        ])) return 4;

        if (in_array($pekerjaan, [
            'PEDAGANG', 'PETANI', 'TANI',
            'PEKERJAAN INFORMAL MENENGAH', 'INFORMAL MENENGAH'
        ])) return 3;

        if (in_array($pekerjaan, [
            'BURUH HARIAN', 'BURUH LEPAS', 'NELAYAN', 'SOPIR',
            'PEKERJAAN INFORMAL RENDAH', 'INFORMAL RENDAH'
        ])) return 2;

        if (in_array($pekerjaan, ['TIDAK BEKERJA', 'BELUM/TIDAK BEKERJA'])) return 1;

        return 1;
    }

    // Sarjana=5, SMA=4, SMP=3, SD=2, Tidak Sekolah=1
    private function mapPendidikanOrtu(?string $pendidikan): int
    {
        ...
        if (in_array($pendidikan, ['SARJANA', 'S1', 'S2', 'S3']))       return 5;
        if (in_array($pendidikan, ['SMA', 'SLTA', 'SMK', 'MA']))        return 4;
        if (in_array($pendidikan, ['SMP', 'SLTP', 'MTS']))              return 3;
        if (in_array($pendidikan, ['SD', 'MI']))                         return 2;
        if (in_array($pendidikan, ['TIDAK SEKOLAH', 'BELUM SEKOLAH']))  return 1;
    ...
    private function quartileExc(array $data, float $quart): float
    {
        ...
        if ($rank <= 1)      return (float) $data[0];
        if ($rank >= $count) return (float) $data[$count - 1];
    ...
    // < Q1 = Tinggi, Q1–Q3 = Sedang, >= Q3 = Rendah
    private function kategoriRisikoQuartile(float $total, float $q1, float $q3): string
    {
        if ($total < $q1)  return 'Tinggi';
        if ($total >= $q3) return 'Rendah';
        return 'Sedang';
    }
